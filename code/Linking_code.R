...
plot_boat_port_lenth <- function(path, blist, year){
  df <- read.csv(file.path(path)) %>% group_by(Vessel.ADFG.Number) %>% filter(any(Vessel.ADFG.Number %in% blist)) %>% ungroup()#filter VESSELS (not permits) that have active salmon permits in 2018 (in the blist)
  plot1 <- {df %>%
      ungroup() %>% 
      ggplot(aes(CFEC.Vessel.Length)) +geom_bar(aes(weight = Pounds..Detail.)) + 
      xlim(0,100)+
      xlab("Vessel length")+
      ylab("Annual Mass of Landings")+
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
      ggtitle(paste0("Catch by Vessel Length (Salmon +), ", year))}
  plot2 <- {df %>%
      ungroup() %>% 
      ggplot(aes(Port.Name)) +geom_bar(aes(weight = Pounds..Detail.)) + 
      xlab("Landing Port")+
      ylab("Annual Mass of Landings")+
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
      ggtitle(paste0("Catch by Landing Port (Salmon +), ", year))}
  
  return(plot1/plot2) # requires 'patchwork' package
}

plot_boat_port_lenth(path = "./../../../2018 fish ticket data (revised 11-1-22).csv", blist = boatlist_onlysalmon$Vessel.ADFG.Number, year = 2018)
plot_boat_port_lenth(path = "./../../../2018 fish ticket data (revised 11-1-22).csv", blist = boatlist$Vessel.ADFG.Number, year = 2018)
