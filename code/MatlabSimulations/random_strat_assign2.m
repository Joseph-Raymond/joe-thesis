...
for i = 1:numIterations
    ...
    rev_store = zeros(numYears, 9);
    firstyear_strat = fishery1_strats(i);
    %specialist years
    for j = 1:numYears
    ...
    firstyear_strat = fishery2_strats(i);
    for j = 1:numYears
    %simulate for numYears and store the  rev_temp with a shock each year
        a1_temp = a1 - 2 + 4 * rand; %adding a random variable ~ Unif[-2,2]
        a2_temp = a2 - 2 + 4 * rand;
    ...
    firstyear_strat = fishery3_strats(i);
    for j = 1:numYears
    %simulate for numYears and store the  rev_temp with a shock each year
        a1_temp = a1 - 2 + 4 * rand; %adding a random variable ~ Unif[-2,2]
        a2_temp = a2 - 2 + 4 * rand;
...
title("CV change (Fixed random strategy)")
...
title("Var/Mean (Fixed random strategy)")
%HHI versus CV
gscatter(temp.HHI, temp.CV, temp.Strategy)
xlabel("HHI")
ylabel("CV")
title("CV / HHI (Fixed random strategy)")
...
title("Income CV change (Fixed random strategy)")

%HHI versus CV
gscatter(temp.HHI, temp.IncomeCV, temp.Strategy)
xlabel("HHI")
ylabel("Income CV")
title("Income CV / HHI (Fixed random strategy)")
